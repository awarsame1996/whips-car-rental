...
export const VehiclesPage = () => {
  ...
  const filterByMake = (filteredInput) => {
    ...
    let filteredCars = [];
    if (filteredInput) {
      filteredCars = data.cars.filter((car) => car.make === `${filteredInput}`);
  ...
  const sortByPrice = (filteredInput) => {
    ...
    if (filteredInput === 'low') {
      ...
      let sortedList = [...carList].sort((a, b) => a.dailyPrice - b.dailyPrice);
      setFilteredList(sortedList);
    }

    if (filteredInput === 'high') {
      let sortedList = [...carList].sort((a, b) => b.dailyPrice - a.dailyPrice);
